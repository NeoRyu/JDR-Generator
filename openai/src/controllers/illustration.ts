...
export const generateImage = async (req: Request, res: Response): Promise<void> => {
    ...
    while (retryCount < maxRetries) {
        try {
            ...
            const response: FetchResponse = await fetch('https://api.openai.com/v1/images/generations', {
                method: 'POST',
                headers: headers,
                body: JSON.stringify({
                    model: dallEModel,
            ...
            const data: OpenAIImageGenerationResponse = await response.json() as OpenAIImageGenerationResponse; // Cast the response to our interface

            if (response.ok && data.data && data.data.length > 0 && data.data[0].b64_json) {
                ...
                if (response.status === 500 || response.status === 503) {
                    console.log(`OpenAI API Server Error (${response.status}), retrying (${retryCount + 1}/${maxRetries})...`);
                    await new Promise(resolve => setTimeout(resolve, (retryCount + 1) * 1000));
                    retryCount++;
                    continue;
                } else {
                    res.status(response.status || 500).json({ message: "Image generation failed", error: data.error ? data.error.message : "Unknown error" });
                    return;
                }
            }
