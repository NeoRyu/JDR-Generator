...
export const generateImage = async (req: Request, res: Response): Promise<void> => {
    ...
    while (retryCount < maxRetries) {
        try {
            ...
            if (err instanceof GoogleGenerativeAIFetchError && err.status === 503) {
                console.log(`Service Unavailable (503), retrying (${retryCount + 1}/${maxRetries})...`);
                await new Promise(resolve => setTimeout(resolve, (retryCount + 1) * 1000)); // Délai exponentiel (1s, 2s, 3s)
                retryCount++;
            } else {
    ...
    res.status(503).json({ message: "Image generation service unavailable after multiple retries." });
};