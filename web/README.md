...
- **Services et leurs fonctions :**
  ...
  - **`deleteCharacter.service.ts` :**
    ...
    const API_BASE_URL =
      import.meta.env.VITE_API_URL || "http://localhost:8080"; // Fallback
  ...
  - **`createCharacter.service.ts` :**
    ...
    const API_BASE_URL =
      import.meta.env.VITE_API_URL || "http://localhost:8080"; // Fallback pour le dev local
  ...
  - **`updateCharacter.service.ts` :**
    ...
    function validateData(characterFull: CharacterFull) {
      if (
        !characterFull ||
        !characterFull.details ||
        !characterFull.details.id
      ) {
      ...
      const updateCharacter = async (characterFull: CharacterFull) => {
        try {
          ...
          const response = await axios.put(
            `${API_BASE_URL}/characters/details/${characterFull.details.id}`,
            cleanedData,
          );
