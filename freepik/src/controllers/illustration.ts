...
export const generateImage = async (
    ...
    try {
      ...
      if (response.ok && data?.data?.task_id) {
        ...
        if (data?.data?.task_id) {
          taskId = data.data.task_id;
          console.log(
            `Freepik task created, but response.ok was false. Task ID: ${taskId}`,
          );
          ...
          console.error(
            "Freepik API error during task creation. Full response data:",
  ...
  if (!taskId) {
    ...
    res.status(500).json({
      message:
        "Freepik image generation service unavailable (task creation failed).",
    });
    return;
  }
  ...
  while (retryCount < pollingMaxAttempts) {
    try {
      console.log(
        `Polling Freepik task status for Task ID: ${taskId} (Attempt ${retryCount + 1}/${pollingMaxAttempts})`,
      );
      const response = await fetch(
      ...
      if (response.ok && data?.data) {
        ...
        if (status === "COMPLETED") {
          if (data.data.generated && data.data.generated.length > 0) {
            ...
            try {
              ...
              console.error(
                `Error downloading or converting image from URL ${imageUrl}:`,
                downloadError,
              );
              res.status(500).json({
                message:
                  "Image generation failed: could not download or convert image.",
            ...
            if (imageBase64) {
              ...
              res.status(500).json({
                message:
                  "Image generation failed: no image data after conversion.",
              });
              return;
            }
          } else {
            console.error(
              `Freepik API returned 'COMPLETED' status but no image URLs for task ${taskId}.`,
            );
            res.status(500).json({
          ...
          console.error(
            `Freepik task ${taskId} failed. Error: ${JSON.stringify(data.data.error)}`,
          ); // <-- MODIFIÉ ICI
          res.status(500).json({
            message: "Image generation failed.",
            error: data.data.error
              ? data.data.error.message
              : "Unknown error during generation.", // <-- MODIFIÉ ICI
          });
          return;
        } else {
          // IN_PROGRESS ou autre statut intermédiaire
        ...
        console.error(
          "Freepik API error or unexpected response structure during task status check:",
          JSON.stringify(data, null, 2),
        );
        await new Promise((resolve) => setTimeout(resolve, pollingDelayMs));
        retryCount++;
      ...
      console.error(
        `Error during Freepik task status check API call for task ${taskId}:`,
        err,
      );
      await new Promise((resolve) => setTimeout(resolve, pollingDelayMs));
      retryCount++;
  ...
  console.error(
    `Image generation polling failed after ${pollingMaxAttempts} attempts for task ${taskId}.`,
  );
  res.status(500).json({
    message:
      "Image generation service timed out or failed after multiple retries.",
  });
};
