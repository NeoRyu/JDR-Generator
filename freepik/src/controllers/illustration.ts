...
if (!process.env.API_KEY) {
  console.error(
      "API_KEY environment variable is not set. Freepik API calls will fail.",
  );
}
...
interface FreepikTaskStatusResponse {
  ...
  generated?: string[]; // URLs des images générées
  error?: {
    code: string;
    message: string;
  };
  ...
  while (retryCount < maxRetries) {
    try {
      ...
      if (response.ok && data && data.task_id) {
        ...
        console.error("Freepik API error during task creation:", data);
        // Les erreurs 500 et 503 sont des erreurs serveur qui peuvent être retentées
        if (response.status === 500 || response.status === 503) {
          ...
          // Autres erreurs (4xx) ne sont pas retentées
      ...
      if (response.ok && data) {
        ...
        if (status === 'COMPLETED') {
          if (data.generated && data.generated.length > 0) {
            const imageUrl = data.generated[0]; // Prend la première image générée
            ...
            if (imageBase64) {
              // Réponse conforme à votre structure attendue
              res.status(200).json({
                image: imageBase64,
                message: "Image generation processed by Freepik", // Message adapté
              });
              return;
            } else {
              res.status(500).json({
          ...
          console.error(`Freepik task ${taskId} failed. Error: ${JSON.stringify(data.error)}`);
          res.status(500).json({
            message: "Image generation failed.",
            error: data.error ? data.error.message : "Unknown error during generation.",
          });
          return;
        ...
        console.error("Freepik API error during task status check:", data);
        await new Promise((resolve) => setTimeout(resolve, pollingDelayMs));
        retryCount++;
      }
    } catch (err: any) {
