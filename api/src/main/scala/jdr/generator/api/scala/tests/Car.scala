...
class Car(test: String = "undefined") extends Vehicle(test) with osef {
  ...
  def calcTotalCost(nbCar: Int, cost: Double = 25000.0): Unit = {
    val subTotal = (nb: Int) => {
      cost * nb
    }
    ...
    // FONCTIONS COMPOSEES :
    val total1 = (calcTva compose subTotal)(nbCar)
    ...
    println(s"Prix pour l'achat de $nbCar : $total1 || $total2 (les deux montant doivent être identique)")
  }
}
...
object Car {
  ...
  def main(args: Array[String]): Unit = {
    ...
    voiture.vroom()
    voiture.calcTotalCost(2) // on veux acheter deux voitures
