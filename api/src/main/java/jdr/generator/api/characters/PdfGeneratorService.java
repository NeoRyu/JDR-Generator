...
public class PdfGeneratorService {
    ...
    try {
      FontFactory.register(
          Objects.requireNonNull(
                  PdfGeneratorService.class.getResource(
                      "/fonts/playwrite-fr-moderne-extralight.ttf"))
              .getFile(),
          "GooglePlayWriteFrModerneExtraLight");
      FontFactory.register(
          Objects.requireNonNull(
                  PdfGeneratorService.class.getResource("/fonts/playwrite-fr-moderne-light.ttf"))
              .getFile(),
          "GooglePlayWriteFrModerneLight");
      FontFactory.register(
          Objects.requireNonNull(
                  PdfGeneratorService.class.getResource("/fonts/playwrite-fr-moderne-regular.ttf"))
              .getFile(),
          "GooglePlayWriteFrModerneRegular");
      FontFactory.register(
          Objects.requireNonNull(
                  PdfGeneratorService.class.getResource("/fonts/playwrite-fr-moderne-thin.ttf"))
              .getFile(),
          "GooglePlayWriteFrModerneThin");
  ...
  private static class CharacterPdfPageEvent extends PdfPageEventHelper {
    ...
    public CharacterPdfPageEvent(
        Image image, CharacterDetailsEntity details, CharacterContextEntity context) {
      ...
      try {
        ...
        this.footerBaseFont =
            FontFactory.getFont("GooglePlayWriteFrModerneThin", 10, Font.UNDERLINE, Color.BLACK)
                .getBaseFont();
    ...
    // Méthode est appelée quand le document est fermé, c'est le moment de remplir le
    // PdfTemplate
    @Override
    ...
    public void onEndPage(PdfWriter writer, Document document) {
      ...
      PdfContentByte directContent =
          writer.getDirectContent(); // Pour le texte (en-tête/pied de page)
      // --- 1. Dessin de l'arrière-plan (comme avant) ---
      try {
        if (backgroundImage != null) {
          backgroundImage.scaleAbsolute(
      ...
      if (writer.getPageNumber() > 1 && characterDetails != null && characterContext != null) {
        String headerText =
            characterDetails.getName().toUpperCase()
                + " / "
        ...
        ColumnText.showTextAligned(
            directContent, Element.ALIGN_CENTER, new Phrase(headerText, headerFont), x, y, 0);
      }
      ...
      if (footerBaseFont != null) { // S'assure que la police a été chargée
        ...
        float estimatedTotalPagesWidth =
            footerBaseFont.getWidthPoint(
        ...
        LOGGER.warn(
            "La police du pied de page n'a pas été chargée, le pied de page ne sera pas dessiné.");
      }
    }
  }
      ...
      if (cellBackgroundImage != null) {
        try {
          ...
          cellBackgroundImage.setAbsolutePosition(
              position.getLeft() - offsetX, position.getBottom() - offsetY);
          canvas.addImage(cellBackgroundImage);
        } catch (DocumentException e) {
          LOGGER.error(
              "Erreur lors de l'ajout de l'image de fond de cellule au PDF: " + e.getMessage());
        }
      }
    }
  }
    ...
    public void cellLayout(PdfPCell cell, Rectangle position, PdfContentByte[] canvases) {
      ...
      if (cellBackgroundImage != null) {
        try {
          ...
          cellBackgroundImage.setAbsolutePosition(imgWidth / 2, position.getBottom());
          canvas.addImage(cellBackgroundImage);
        } catch (DocumentException e) {
          LOGGER.error(
              "Erreur lors de l'ajout de l'image de fond " + "de cellule au PDF: {}",
              e.getMessage());
        }
      }
    }
  ...
  private Paragraph context(final String label, final String text, final float fontSize) {
  ...
  private Phrase contextContentLabel(final String label, final float fontSize) {
    final Font underlinedLabelFont =
        FontFactory.getFont(FontFactory.COURIER_BOLD, fontSize, Font.UNDERLINE, Color.BLACK);
    if (label == null || label.isEmpty()) {
      return new Phrase("", underlinedLabelFont);
    }
    return new Phrase(
        label.substring(0, 1).toUpperCase() + label.substring(1), underlinedLabelFont);
  }

  private Phrase contextContent(final String text, final float fontSize) {
    final Font infoFont = FontFactory.getFont(FontFactory.COURIER_BOLD, fontSize, Color.BLACK);
    if (text == null || text.isEmpty()) {
      return new Phrase("", infoFont);
    }
    return new Phrase(text.substring(0, 1).toUpperCase() + text.substring(1), infoFont);
  }

  private Paragraph paragraph(
      final int level, final String label, final String text, final float fontSize) {
    Paragraph p = new Paragraph();
    p.setSpacingAfter(5f);
    p.setLeading(20f);
  ...
  private Phrase paragraphContentLabel(final String label, final float fontSize) {
    final Font underlinedLabelFont =
        FontFactory.getFont("GooglePlayWriteFrModerneRegular", fontSize, Font.BOLD, Color.BLACK);
    if (label == null || label.isEmpty()) {
      return new Phrase("", underlinedLabelFont);
    }
    return new Phrase(
  ...
  private Phrase paragraphContent(final String text, final float fontSize) {
    final Font infoFont =
        FontFactory.getFont("GooglePlayWriteFrModerneThin", fontSize, Color.BLACK);
    if (text == null || text.isEmpty()) {
      return new Phrase("", infoFont);
    }
    return new Phrase(text.substring(0, 1).toUpperCase() + text.substring(1), infoFont);
  ...
  public byte[] generateCharacterPdf(Long characterId) throws DocumentException {
    ...
    CharacterDetailsEntity c_details = this.characterDetailsService.findById(characterId);
    CharacterContextEntity c_context =
        this.characterContextService.findById(c_details.getContextId());
    CharacterIllustrationEntity c_illustration =
        characterIllustrationService.findByCharacterDetailsId(characterId);
    CharacterJsonDataEntity c_json_data =
        characterJsonDataService.findByCharacterDetailsId(characterId).orElse(null);
    ...
    try {
      ...
      try {
        ...
        InputStream imageStream = getClass().getClassLoader().getResourceAsStream(bgImg_Sheet);
        if (imageStream != null) {
      ...
      CharacterPdfPageEvent event = new CharacterPdfPageEvent(parchmentImage, c_details, c_context);
      ...
      Paragraph title =
          new Paragraph(
              c_details.getName().toUpperCase(),
              FontFactory.getFont(FontFactory.TIMES_BOLD, 24, Font.UNDERLINE, Color.BLACK));
      title.setAlignment(Element.ALIGN_CENTER);
      textCell.addElement(title);
      textCell.addElement(new Paragraph(CRLF));
      textCell.addElement(new Paragraph(CRLF));
      // INFOS PRINCIPALES (CONTEXTE + AGE)
      textCell.addElement(context("Univers", c_context.getPromptSystem(), 14));
      textCell.addElement(new Paragraph(CRLF));
      textCell.addElement(context("Classe", c_context.getPromptClass(), 14));
      textCell.addElement(context("Race", c_context.getPromptRace(), 14));
      textCell.addElement(context("Sexe", c_context.getPromptGender(), 14));
      textCell.addElement(context("Age", String.valueOf(c_details.getAge()), 14));
      textCell.addElement(new Paragraph(CRLF));
      // Charger une image splash separator
      Image splashCellBackgroundImage = null;
      try {
        InputStream splashImageStream =
            getClass().getClassLoader().getResourceAsStream(bgImg_Splash);
      ...
      try {
        InputStream portraitImageStream =
            getClass().getClassLoader().getResourceAsStream(bgImg_Portrait);
      ...
      try {
        if (c_illustration != null
            && c_illustration.getImageBlob() != null
          ...
          float documentWidth =
              document.getPageSize().getWidth() / 10 * 4
                  - document.leftMargin()
          ...
          imageCell.addElement(new Paragraph("Aucun portrait n'a été généré...", infoError));
        }
      } catch (IOException e) {
        LOGGER.error("Impossible d'ajouter l'image au PDF: " + e.getMessage());
        imageCell.addElement(new Paragraph("ERREUR : Portrait non disponible.", infoError));
      }
      ...
      // DETAILS
      document.add(paragraph(0, "Lieu de naissance", c_details.getBirthPlace(), 9));
      document.add(paragraph(0, "Lieu de résidence", c_details.getResidenceLocation(), 9));
      document.add(paragraph(0, "Raison de la résidence", c_details.getReasonForResidence(), 9));
      document.add(paragraph(0, "Climat", c_details.getClimate(), 9));
      document.add(paragraph(0, "Problèmes communs", c_details.getCommonProblems(), 9));
      document.add(paragraph(0, "Routine journalière", c_details.getDailyRoutine(), 9));
      document.add(
          paragraph(0, "Parents en vie ?", c_details.getParentsAlive() ? "Oui" : "Non", 9));
      document.add(paragraph(0, "Détails sur les parents", c_details.getDetailsAboutParents(), 9));
      document.add(
          paragraph(
              0, "Sentiments par rapport aux parents", c_details.getFeelingsAboutParents(), 9));
      document.add(paragraph(0, "Fraternité ?", c_details.getSiblings(), 9));
      document.add(paragraph(0, "Enfance du personnage", c_details.getChildhoodStory(), 9));
      document.add(paragraph(0, "Amis d'enfance", c_details.getYouthFriends(), 9));
      document.add(paragraph(0, "Animal de compagnie", c_details.getPet(), 9));
      document.add(paragraph(0, "État matrimonial", c_details.getMaritalStatus(), 9));
      document.add(paragraph(0, "Type de conjoint", c_details.getTypeOfLover(), 9));
      document.add(paragraph(0, "Histoire conjugale", c_details.getConjugalHistory(), 9));
      document.add(paragraph(0, "Nombre d'enfants", c_details.getChildren().toString(), 9));
      document.add(paragraph(0, "Education", c_details.getEducation(), 9));
      document.add(paragraph(0, "Profession", c_details.getProfession(), 9));
      document.add(paragraph(0, "Raison de la profession", c_details.getReasonForProfession(), 9));
      document.add(paragraph(0, "Préférences de travail", c_details.getWorkPreferences(), 9));
      document.add(
          paragraph(0, "Changement souhaité dans le monde", c_details.getChangeInWorld(), 9));
      document.add(paragraph(0, "Souhait personnel", c_details.getChangeInSelf(), 9));
      document.add(paragraph(0, "Objectif de vie", c_details.getGoal(), 9));
      document.add(paragraph(0, "Raison de l'objectif", c_details.getReasonForGoal(), 9));
      document.add(paragraph(0, "Obstacle majeur", c_details.getBiggestObstacle(), 9));
      document.add(paragraph(0, "Surmonter les obstacles", c_details.getOvercomingObstacle(), 9));
      document.add(paragraph(0, "Plan en cas de succès", c_details.getPlanIfSuccessful(), 9));
      document.add(paragraph(0, "Plan en cas d'échec", c_details.getPlanIfFailed(), 9));
      document.add(paragraph(0, "Description de la personne", c_details.getSelfDescription(), 9));
      document.add(paragraph(0, "Signe distinctif", c_details.getDistinctiveTrait(), 9));
      document.add(paragraph(0, "Description Physique", c_details.getPhysicalDescription(), 9));
      document.add(paragraph(0, "Préférence vestimentaire", c_details.getClothingPreferences(), 9));
      document.add(paragraph(0, "Phobies", c_details.getFears(), 9));
      document.add(paragraph(0, "Nourriture favorite", c_details.getFavoriteFood(), 9));
      document.add(paragraph(0, "Loisirs", c_details.getHobbies(), 9));
      document.add(paragraph(0, "Activités de loisir", c_details.getLeisureActivities(), 9));
      document.add(paragraph(0, "Compagnons idéaux", c_details.getIdealCompany(), 9));
      document.add(
          paragraph(0, "Attitude envers le groupe", c_details.getAttitudeTowardsGroups(), 9));
      document.add(paragraph(0, "Vision du monde", c_details.getAttitudeTowardsWorld(), 9));
      document.add(
          paragraph(0, "Attitude envers les autres", c_details.getAttitudeTowardsPeople(), 9));
  ...
  /**
   * Méthode récursive pour ajouter les données JSON au document PDF. Imite la logique d'affichage
   * hiérarchique du frontend.
   */
  private void addJsonDataToPdf(Document document, Map<String, Object> jsonData, int level)
      ...
      if (value instanceof ArrayList<?>) {
        ...
        for (int i = 0; i < list.size(); i++) {
          ...
          if (item instanceof Map) {
            ...
            document.add(paragraph(level, String.valueOf(item), "", calcFontSize(14, level)));
          }
        }
